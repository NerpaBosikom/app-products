...
function loadFromLocalStorage(): {products: Product[], likes: Set<number>} | null {
  try {
    const raw = localStorage.getItem(persistKey)
    if (!raw) return null
    const parsed = JSON.parse(raw)
    return { products: parsed.products || [], likes: new Set<number>(parsed.likes || []) }
  } catch { return null }
}
  ...
  async fetchAll() {
    ...
    try {
      ...
      if (restored && restored.products.length) {
        set({ products: restored.products, likes: restored.likes, loading: false })
        return
      }
